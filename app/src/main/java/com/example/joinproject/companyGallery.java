...
public class companyGallery extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.company_gallery);

        WriteGallery=findViewById(R.id.WriteGallery);

        if(mauth.getUid().equals("0m4drrIAiYO3rB9f9BQhoQhpKBT2")){
            WriteGallery.setVisibility(View.VISIBLE);
        }
        WriteGallery.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                startActivity(new Intent(v.getContext(), plusGallery.class));
            }
        });
        ...
        databaseReference = database.getReference("Gallery");
        databaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                for (DataSnapshot snapshot : dataSnapshot.getChildren()) {//반복문으로 데이터 List를 추출해냄
                    Gallery gallery = snapshot.getValue(Gallery.class); //만들어뒀던 Company객체에 데이터를 담는다
        ...
        adapter = new GalleryAdapter(arrayList, this);
