...
public class companyGallery4 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.company_gallery4);

        WriteGallery=findViewById(R.id.WriteGallery);

        if(mauth.getUid().equals("EM95VCkH10Rqat7BxLLop0WFERJ2")){
            WriteGallery.setVisibility(View.VISIBLE);
        }
        WriteGallery.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                startActivity(new Intent(v.getContext(), plusGallery4.class));
            }
        });
        ...
        databaseReference = database.getReference("Gallery4");
        databaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                for (DataSnapshot snapshot : dataSnapshot.getChildren()) {//반복문으로 데이터 List를 추출해냄
                    Gallery4 gallery = snapshot.getValue(Gallery4.class); //만들어뒀던 Company객체에 데이터를 담는다
        ...
        adapter = new GalleryAdapter4(arrayList, this);
